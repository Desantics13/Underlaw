...
import { BarChart3, LogOut, Bell, Settings, CheckCircle, Clock, User, X, Eye } from 'lucide-react';
...
const ESTADO_COLORS = {
  Pagado: { bg: 'rgba(16,185,129,0.1)', color: '#10b981' },
  Pendiente: { bg: 'rgba(245,158,11,0.1)', color: '#f59e0b' },
  Rechazado: { bg: 'rgba(244,63,94,0.1)', color: '#f43f5e' },
  Anulado: { bg: 'rgba(148,163,184,0.1)', color: '#94a3b8' },
  Error: { bg: 'rgba(244,63,94,0.1)', color: '#f43f5e' }
};
...
const Admin = () => {
  ...
  useEffect(() => {
    ...
    const stored = JSON.parse(localStorage.getItem('underlaw_notificaciones')) || [];
  ...
  const markAllRead = () => {
    ...
    localStorage.setItem('underlaw_notificaciones', JSON.stringify(updated));
  };

  const clearNotifications = () => {
    setNotificaciones([]);
    localStorage.removeItem('underlaw_notificaciones');
    setShowNotifications(false);
  };

  const unreadCount = notificaciones.filter(n => !n.leida).length;
  const totalIngresos = pedidos.filter(p => p.estado === 'Pagado').reduce((sum, p) => sum + Number(p.precio), 0);
                        ...
                        notificaciones.map(n => (
                          <div key={n.id} style={{ padding: '1rem 1.25rem', borderBottom: '1px solid rgba(51,65,85,0.5)', background: n.leida ? 'transparent' : 'rgba(59,130,246,0.05)', display: 'flex', gap: '0.75rem', alignItems: 'flex-start' }}>
                            ...
                            <div>
                              ...
                              <p style={{ fontSize: '0.75rem', color: '#64748b', marginTop: '0.25rem' }}>{n.fecha}</p>
                            </div>
                          </div>
                        ))
                      )}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '600px' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #334155', color: '#94a3b8', fontSize: '0.85rem' }}>
                ...
                <th style={{ paddingBottom: '1rem', fontWeight: '500' }}>Acciones</th>
              </tr>
            </thead>
            <tbody>
              ...
              {pedidos.length === 0 && (
                <tr><td colSpan="6" style={{ textAlign: 'center', padding: '3rem 0', color: '#64748b' }}>No hay pedidos registrados aún.</td></tr>
              )}
            </tbody>
          </table>
          </div>
        ...
        {selectedPedido && (
          <>
            ...
            <motion.div
              ...
              style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '90%', maxWidth: '500px', background: '#1e293b', border: '1px solid #334155', borderRadius: '16px', padding: '2.5rem', zIndex: 301 }}
            >
