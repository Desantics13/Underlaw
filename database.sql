...
-- -------------------------------------------------------------------------
-- ACTUALIZACIÓN: Seguridad del Admin (login con JWT) + Secciones de la
-- Colección + campos nuevos de producto/lanzamiento (descripción, detalle,
-- tallas, sección). Detalle completo y comentado en migracion-admin-panel.sql
-- (créalo desde ahí en Railway/Workbench; acá solo queda el registro).
-- La contraseña del Admin YA NO vive en el código: se compara en el backend
-- contra la variable de entorno ADMIN_PASSWORD y el login devuelve un JWT
-- firmado con ADMIN_JWT_SECRET (ver backend/controllers/AdminController.js).
-- -------------------------------------------------------------------------
...
-- -------------------------------------------------------------------------
-- ACTUALIZACIÓN: Inventario real por producto y talla, conectado con las
-- ventas. Detalle completo y comentado en migracion-inventario.sql (créalo
-- desde ahí en Railway/Workbench; acá solo queda el registro).
--
-- El interruptor manual "disponible" (true/false) de cada talla pasa a ser
-- una cantidad real en stock: cantidad = NULL sigue significando "sin contar"
-- (se vende sin límite, como antes); cantidad = 0, 1, 2... es el stock real.
...
-- Marca de "ya descontado" en el pedido (tabla "producto" = pedidos/compras),
-- para que el descuento de inventario nunca se aplique dos veces al mismo pedido.
ALTER TABLE producto ADD COLUMN inventario_descontado TINYINT(1) NOT NULL DEFAULT 0;

-- Detalle estructurado de qué se compró en cada pedido (producto + talla +
-- cantidad), para descontar el inventario exacto. Se llena al iniciar el pago.
